...
async def logUserBot():
    load_dotenv()
    api_id = int(25851747)
    api_hash = "b4bb16507d01a0e9ef974d1e686287ec"
    phone_number = "51988146893"
    ...
    await client.send_message("@spamlester2", f'<b>Bot encendido</b>', parse_mode="HTML")
    ...
    excluded_group_ids = [-1002403730314,-4794938646,-1001907073788]
    ...
    while True:
        ...
        try:
            await client.send_message("@spamlester2", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>", parse_mode="HTML")
        except:
            pass
                        ...
                        try:
                            ...
                            if "CHAT_SEND_PHOTOS_FORBIDDEN" in str(error):
                                ...
                                try:
                                    ...
                                    await client.send_message("@spamlester2", f'<b>Enviado solo texto a {i["group_id"]}</b> - <code>{i["group_name"]}</code>', parse_mode="HTML")
                                except Exception as new_error:
                                    await client.send_message("@spamlester2", f'<b>Error enviando solo texto a {i["group_id"]}</b> - <code>{i["group_name"]}</code>\nCausa: {new_error}', parse_mode="HTML")
                            else:
                                await client.send_message("@spamlester2", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}</code>\nCausa: {error}', parse_mode="HTML")
                                resultado = False
                        if resultado:
                            await client.send_message("@spamlester2", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>', parse_mode="HTML")  
                            await asyncio.sleep(10)
                        if j == 3: break
            await client.send_message("@spamlester2", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
