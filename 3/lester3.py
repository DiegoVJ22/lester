...
async def logUserBot():
    load_dotenv()
    api_id = int(25499073)
    api_hash = "3b0c25dfb837280d51f09029754f098b"
    phone_number = "51978064793"
    ...
    await client.send_message("@spamlester3", f'<b>Bot encendido</b>', parse_mode="HTML")
    ...
    excluded_group_ids = [-4794938646,-1002496536745,-1001907073788]
    ...
    while True:
        ...
        try:
            await client.send_message("@spamlester3", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>", parse_mode="HTML")
        except:
            pass
                        ...
                        try:
                            ...
                            if "CHAT_SEND_PHOTOS_FORBIDDEN" in str(error):
                                ...
                                try:
                                    ...
                                    await client.send_message("@spamlester3", f'<b>Enviado solo texto a {i["group_id"]}</b> - <code>{i["group_name"]}</code>', parse_mode="HTML")
                                except Exception as new_error:
                                    await client.send_message("@spamlester3", f'<b>Error enviando solo texto a {i["group_id"]}</b> - <code>{i["group_name"]}</code>\nCausa: {new_error}', parse_mode="HTML")
                            else:
                                await client.send_message("@spamlester3", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}</code>\nCausa: {error}', parse_mode="HTML")
                                resultado = False
                        if resultado:
                            await client.send_message("@spamlester3", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>', parse_mode="HTML")  
                            await asyncio.sleep(10)
                        if j == 3: break
            await client.send_message("@spamlester3", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
