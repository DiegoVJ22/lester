...
async def logUserBot():
    load_dotenv()
    api_id = int(23217710)
    api_hash = "9da8748348ed580cc76338613e0b3420"
    phone_number = "51979869236"
    ...
    await client.send_message("@spamlester4", f'<b>Bot encendido</b>', parse_mode="HTML")
    ...
    excluded_group_ids = [-1002348082792,-4794938646,-1001907073788]
    ...
    while True:
        ...
        try:
            await client.send_message("@spamlester4", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>", parse_mode="HTML")
        except:
            pass
                        ...
                        try:
                            ...
                            if "CHAT_SEND_PHOTOS_FORBIDDEN" in str(error):
                                ...
                                try:
                                    ...
                                    await client.send_message("@spamlester4", f'<b>Enviado solo texto a {i["group_id"]}</b> - <code>{i["group_name"]}</code>', parse_mode="HTML")
                                except Exception as new_error:
                                    await client.send_message("@spamlester4", f'<b>Error enviando solo texto a {i["group_id"]}</b> - <code>{i["group_name"]}</code>\nCausa: {new_error}', parse_mode="HTML")
                            else:
                                await client.send_message("@spamlester4", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}</code>\nCausa: {error}', parse_mode="HTML")
                                resultado = False
                        if resultado:
                            await client.send_message("@spamlester4", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>', parse_mode="HTML")  
                            await asyncio.sleep(10)
                        if j == 3: break
            await client.send_message("@spamlester4", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
